...
_sd.path_ is a character string that is a URI identifying the input source
or the input source kind.
This string is valid until interpretation of this input source is finished.
...
- If the input source is a file, _sd.path_ is a full path or URI
by which the file can be opened with `open-file`.
Whether the URI maps to a file in the local file system or to a remote resource
